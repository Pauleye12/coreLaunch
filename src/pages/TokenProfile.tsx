...
import { useAccount, useBlock, useClient, useConfig, useWriteContract } from "wagmi";
          ...
          <div className="bg-[#1C4141] text-white flex flex-col gap-5 p-5 md:max-w-[50%] w-full rounded-2xl">
            <div className="flex gap-3 items-center ">
              ...
              <div className="flex flex-col items-start gap-1 ">
                ...
                <p className="text-[#A7A7A7] text-sm  ">{formatDate(token?.timestamp || "")}</p>
              </div>
            </div>
            ...
            <div className="flex mt-4 md:m-0 flex-col md:flex-row justify-between gap-3 w-full md:items-center">
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm">
                  Created by:{" "}
                  <span className="ml-[2px] text-[#8BF0B7]">{truncate(token?.creator)}</span>
                </p>
              </div>
          ...
          <div className="w-full overflow-x-auto ">
            <table className="w-full border-separate border-spacing-x-0 border-spacing-y-6 ">
              ...
              {trades?.map((trade, index) => (
                <Tablerow item={trade} key={index} explorerUrl={client?.chain.blockExplorers?.default.url || ""} />
              ))}
            </table>
          </div>
        </div>
